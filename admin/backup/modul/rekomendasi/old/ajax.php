...
if($_POST){
	switch (trim(strip_tags($_POST['a']))) {
		...
		case 'add-hsl-periksa':
			...
			if($sql->error==null){
				...
				for($x=0;$x<count($_POST['jenis_ikan']);$x++){
					$asal_komoditas=(($_POST['asal_komoditas_opt'][$x]=='lainnya')?$_POST['asal_komoditas'][$x]:$_POST['asal_komoditas_opt'][$x]);
					$arr_insert2=array(
						'ref_idikan'=>$_POST['jenis_ikan'][$x],
						'ref_idp'=>$idpengajuan,
						'ref_idperiksa'=>$idperiksa,
						'ref_jns_sampel'=>$_POST['jenis_sampel'][$x],
						'pjg'=>$_POST['pjg'][$x],
						'lbr'=>$_POST['lbr'][$x],
						'berat'=>$_POST['berat'][$x],
						'tot_berat'=>$_POST['berat_tot'][$x],
						'ket'=>$_POST['ket'][$x],
						'date_insert'=>date('Y-m-d H:i:s'),
						'asal_komoditas'=>$asal_komoditas,
						'kuantitas'=>$_POST['kemasan'][$x]
						);
		...
		case 'update-hsl-periksa':
			...
			if($sql->error==null){
				$sql->delete('tb_hsl_periksa',array('ref_idp'=>$idpengajuan,'ref_idperiksa'=>$idperiksa));
				...
				for($x=0;$x<count($_POST['jenis_ikan']);$x++){
					...
					$sql->insert('tb_hsl_periksa',$arr_insert);
				}
				echo json_encode(array("stat"=>true,"msg"=>"Data Berhasil Disimpan."));
			}else{
				echo json_encode(array("stat"=>false,"msg"=>"Aksi Gagal."));
