...
//load list pengajuan
$sql->get_all('tb_barang',array('ref_idphn'=>$idpengajuan),'nm_barang');
$barang=array();
...
$pt=$sql->run("SELECT op.nm_lengkap, op.nip FROM tb_petugas_lap pl JOIN op_pegawai op ON (pl.nip=op.nip) WHERE pl.ref_idp='$idpengajuan'");
if($pt->rowCount()>0){
	$no=0;
...
if($sql->num_rows>0){
	...
	foreach($sql->result as $gbr){
		$nog++;
		$html.='<td><img style="padding:10px" width="100%" src="'.$berkas_admin_foto.$gbr['nm_file'].'"><p>'.$gbr['ket_foto'].'</p></td>';
		if($nog>1){
			if($nog%2==0){
				$html.='</tr><tr>';
			}
