...
function cek_session(){
    ...
    if(!$_COOKIE[$SITE_CONF_AUTOLOAD['cookie']] && !$_SESSION[$SITE_CONF_AUTOLOAD['cookie']]){
        ...
        if($sql -> get_row('op_user',array('idu'=>$username,'pwd'=>md5($userpwd)), '*') ){
            $result = $sql ->result; extract($result);           
            define("USER", TRUE);
            define("U_ID", $idu);
            define("U_NAME", $nm_lengkap);
            define("U_PASS", $pwd);
            define("U_EMAIL", $email);
            define("U_STATUS", $status);
            define("U_JABATAN", $jabatan);
            define("U_LEVEL", $lvl);

            $sql -> get_all('web_meta',array('ref_id'=>U_ID,'meta_option'=>'autoload','meta_group'=>2), array('meta_key `KEY`','meta_value VAL') );
...
function generate_nosurat($jenis,$no,$kode,$tgl){
    ...
    if($jenis!=""){
        ...
        switch ($jenis) {
            case 'st':
                $no_surat="E.B.".$no."/LPSPL.2/PK.242/".$bulantahun;
            break;

            case 'bap':
                $no_surat="E.BAP.".$no."/LPSPL.2/PK.242/".$bulantahun;
            break;

            case 'rek':
                $no_surat="E.".$no."/LPSPL.2/PK.242/".$bulantahun;
            break;
