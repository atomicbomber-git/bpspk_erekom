...
function cek_session(){
    ...
    if(!$_COOKIE[$SITE_CONF_AUTOLOAD['cookie']] && !$_SESSION[$SITE_CONF_AUTOLOAD['cookie']]){
        ...
        list($idp,$uname, $email) = ($_COOKIE[$SITE_CONF_AUTOLOAD['cookie']] ? explode(".", $_COOKIE[$SITE_CONF_AUTOLOAD['cookie']]) : explode(".", $_SESSION[$SITE_CONF_AUTOLOAD['cookie']]));
        ...
        $c=$sql->run("SELECT op.nip,op.nm_lengkap FROM tb_petugas_lap pl JOIN op_pegawai op ON(pl.ref_idpeg=op.idp) WHERE pl.ref_idp='$idp' AND md5(op.email)='$email' LIMIT 1");
        if($c->rowCount()>0){
            $p=$c->fetch();
            $sql -> get_row('tb_permohonan',array('log_u'=>$uname,'status'=>2), array('idp'));
...
function generate_nosurat($jenis,$no,$kode,$tgl){
    $no_surat = "-";
    if($jenis!=""){
